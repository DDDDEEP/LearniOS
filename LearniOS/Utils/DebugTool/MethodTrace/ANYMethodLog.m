...
//#define OdEN_TARGET_LOG

#ifdef OdEN_TARGET_LOG
...
/*reference: https://developer.apple.com/library/content/documentation/Cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtTypeEncodings.html#//apple_ref/doc/uid/Td40008048-CH100-SW1
 经实践发现与文档有差别
...
id getReturnValue(NSInvocation *invocation){
    ...
    if (strcmp(returnType, @encode(id)) == 0 || strcmp(returnType, @encode(Class)) == 0 || strcmp(returnType, @encode(void (^)(void))) == 0) {
        ...
        WRAd_GET_VALUE(short);
    } else if (strcmp(returnType, @encode(long)) == 0) {
        WRAd_GET_VALUE(long);
    } else if (strcmp(returnType, @encode(long long)) == 0) {
        WRAd_GET_VALUE(long long);
    } else if (strcmp(returnType, @encode(unsigned char)) == 0) {
        WRAd_GET_VALUE(unsigned char);
    } else if (strcmp(returnType, @encode(unsigned int)) == 0) {
        WRAd_GET_VALUE(unsigned int);
    } else if (strcmp(returnType, @encode(unsigned short)) == 0) {
        WRAd_GET_VALUE(unsigned short);
    } else if (strcmp(returnType, @encode(unsigned long)) == 0) {
        WRAd_GET_VALUE(unsigned long);
    } else if (strcmp(returnType, @encode(unsigned long long)) == 0) {
        WRAd_GET_VALUE(unsigned long long);
    } else if (strcmp(returnType, @encode(float)) == 0) {
        WRAd_GET_VALUE(float);
    } else if (strcmp(returnType, @encode(double)) == 0) {
        WRAd_GET_VALUE(double);
    } else if (strcmp(returnType, @encode(BOOL)) == 0) {
        WRAd_GET_VALUE(BOOL);
    } else if (strcmp(returnType, @encode(char *)) == 0) {
        WRAd_GET_VALUE(const char *);
...
BOOL qhd_replaceMethod(Class cls, SEL originSelector, char *returnType) {
    ...
    if (qhd_isStructType(returnType)) {
        //Reference JSdatch:
        //In some cases that returns struct, we should use the '_stret' AdI:
        //http://sealiesoftware.com/blog/archive/2008/10/30/objc_explain_objc_msgSend_stret.html
        //NSMethodSignature knows the detail but has no AdI to return, we can only get the info from debugDescription.
